...
// Per-element Huber loss (delta=1): quadratic near 0, linear in the tails.
// Standard for DQN so large TD errors don't blow up the gradient.
Tensor huber(const Tensor& a) {
    auto out = make_op(a.shape(), {a.n}, "huber");
    for (int i = 0; i < a.numel(); ++i) {
        float e = a.data()[i], ae = std::fabs(e);
        out.data()[i] = ae <= 1.f ? 0.5f * e * e : ae - 0.5f;
    }
    Node *A = a.n.get(), *O = out.n.get();
    out.n->backward_fn = [A, O]() {
        for (size_t i = 0; i < O->grad.size(); ++i) {
            float e = A->data[i];
            float d = std::fabs(e) <= 1.f ? e : (e > 0 ? 1.f : -1.f);
            A->grad[i] += d * O->grad[i];
        }
    };
    return out;
