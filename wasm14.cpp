// WebAssembly bindings for World 1-4 (castle). Runs the headless NES + the 1-4
// QNet (live greedy) in the browser. The 1-4 start is reached by an in-core RAM
// level-warp (Env::reset -> death-reload), so NO demo is embedded -- only the net
// (net14.bin). Rebuild with a better net to swap the agent in place. ROM from JS.
#include "nes.h"
#include "mario14.h"
#include "qnet.h"
#include "autograd.h"
...
static mario14::Env g_env;
static QNet*        g_net = nullptr;
static std::vector<uint8_t> g_rgba;
static bool         g_ready = false;
static const int S = mario14::Env::STATE_DIM;
static const int A = mario14::Env::ACTIONS;
...
int wasm_load_rom(const uint8_t* rom, int n) {
    ...
    if (!g_net) { g_net = new QNet(S, A, NET_HID); g_net->load("net14.bin"); }
    g_env.reset();                 // warps to 1-4 (cached after first call)
