...
// Boot to 1-1, then WARP to 1-3: set the stage RAM and force a death so the
// area-load routine re-reads the (now 1-3) area pointer. Re-poke through the
// reload so the load definitely sees 1-3.
void Env::goto_1_3_start() {
    boot(rom_);
    auto poke = [](){
    ...
    nes::ram_write(0x075A, 9);        // lives high so the warp death can't game-over
    poke();
    nes::ram_write(0x000E, 0x06);     // player state = dying -> triggers area reload
    for (int k = 0; k < 600; ++k) {
        poke();                        // hold the target stage through the reload
        nes::set_buttons(0, 0);
        nes::step_frame();
...
bool Env::is_win() const { return float_state() == 0x03; }   // flagpole
...
float Env::step(int action, bool& done) {
    ...
    nes::set_buttons(0, mario13::action_buttons(action));
    ...
    int a = area_index();
    if (a != cur_area_) {
        // area change in 1-3 = level end/transition; rebaseline (no pipe bonus here).
        cur_area_ = a;
        prev_x_ = level_x(); max_x_ = prev_x_; stall_ = 0;
    } else {
        int x = level_x();
        prev_x_ = x;
        // RATCHET progress: only new ground scores; waiting is free (no time penalty),
        // so timing a jump onto a moving lift / waiting out a hazard is not punished.
