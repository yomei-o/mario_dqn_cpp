...
---
## 🟢 2026-07-19: 行動集合を「人間の手」に絞る実験（3行動 vs 5行動を選択可能に）

**ユーザーの洞察**: 人間のマリオ操作は基本ずっと `RIGHT+B`（走る）、穴/敵/土管で `A`（前飛び）、踏むときは右とBを離して `A`。
つまりエージェントの本質的な仕事は「いつ・どうジャンプするか」だけ。→ **行動を3つに絞る**と決定空間が激減する。

- **`MARIO_ACTIONS` ビルド切替**（`mario.h`）: 既定=5（従来の右バイアス）、`-DMARIO_ACTIONS=3` で人間手集合。
  - 3行動: `A_RUN`=R+B / `A_RUNJUMP`=R+A+B / `A_STOMP`=A。`A_RUNRIGHT` エイリアスで両モードのツールが動く。
  - CMake: `mario_dqn`(5行動) と **`mario_dqn3`(3行動)** の2ターゲット。`mario.cpp`の`action_buttons`と
    `mario_dqn.cpp`（finditemサンプラ=一様乱数化、"run right"=`A_RUNRIGHT`）を両対応に。
- **結果（3行動・ゼロ学習, 4ワーカー）**: **探索が ep29 で1-1完全クリア**（`prefix 0 + 513手`＝カリキュラム無しでスタート通し）。
  greedy が短時間で **1524** 到達（旧5行動の数時間相当）。ただし「ブロック階段(~1513)」で頭打ち＋greedy揺れ(retention)は同じ。
  クリアデモ多数キャプチャ→カリキュラム/BC投入も、**BCは探索デモが雑で 900 止まり**（人間の綺麗なデモならBCが効く見込み）。
- **資産**: `warmstarts/demo_clear_1-1_3act.bin`（3行動のスタート通しクリア513手）。
  ※3行動で学習/カリキュラムする時は `demo.bin` にこの3行動デモを使うこと（5行動デモは指標が違い流用不可）。
- **結論**: ユーザー仮説は明確に有効（探索が早々にクリア到達・greedyが素直に伸びる）。確実なスタート通しgreedyクリアは
  「綺麗なクリアデモ→BC」が最短（`mario_bc_cpp` の人間プレイ録画ツールと接続する筋）。
